...
// Attempt to reduce Supabase egress by transforming large image URLs to a smaller size
// Supabase image transformation supports width / quality query params when enabled.
const transformImageUrl = (url: string | undefined, width = 300, quality = 60) => {
  if (!url) return ''
  try {
    // Only transform Supabase public storage URLs
    if (url.includes('/storage/v1/object/public/') && !url.includes('width=')) {
...
export default function SPL02Players() {
  ...
  useEffect(() => {
    const fetchPlayers = async () => {
      try {
        setLoading(true)
        // Fetch static local JSON (does not incur Supabase egress). Consider importing statically if needed.
  ...
  const getCurrentPlayers = () => {
    switch (activeTab) {
      case 'MENS': return mensPlayers
      case 'WOMENS': return womensPlayers
      case 'KIDS': return kidsPlayers
      default: return []
    }
  }

  // Player card component - now with lazy-loading image optimizations
  const PlayerCard = ({ player }: { player: Player }) => {
    const categoryColor = categoryColors[player.CATEGORY as keyof typeof categoryColors] || 'from-blue-500 to-blue-600'
    const optimizedPhotoUrl = transformImageUrl(player['Photo URL'])
    ...
    return (
      <div className="bg-white dark:bg-gray-800 rounded-lg sm:rounded-xl shadow-lg hover:shadow-xl dark:shadow-gray-900/20 dark:hover:shadow-gray-900/30 transition-all duration-300 overflow-hidden border border-gray-100 dark:border-gray-700 hover:scale-105 active:scale-95">
        ...
        <div className="p-3 sm:p-4 space-y-2 sm:space-y-3 bg-white dark:bg-gray-800">
          <div>
            <h3 className="font-bold text-sm sm:text-base md:text-lg text-gray-900 dark:text-white mb-1 leading-tight line-clamp-2">{player['Full Name']}</h3>
          </div>
          <div className="space-y-1.5 sm:space-y-2 text-xs sm:text-sm">
            ...
            <div className="flex justify-between items-center">
              ...
              <a href={`tel:${player['Mobile Number']}`} className="font-semibold hover:text-blue-600 dark:hover:text-blue-300 transition-colors flex items-center space-x-1 active:scale-95 text-gray-900 dark:text-white">
  ...
  const totalPlayers = playerCounts[activeTab]

  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-yellow-50 py-6 sm:py-12">
      <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8">
        <div className="text-center mb-6 sm:mb-8 md:mb-12">
          <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-yellow-600 mb-3 sm:mb-4 px-2">SPL-02 Player List</h1>
          <p className="text-sm sm:text-base md:text-lg text-gray-600 max-w-3xl mx-auto mb-6 sm:mb-8 px-4 leading-relaxed">Meet the talented players competing in Sparsh Premier League Season 2.</p>
        ...
        <div className="text-center mb-6 sm:mb-8">
          <div className="flex flex-wrap justify-center gap-2 sm:gap-4">
            <Link href="/spl02" className="bg-white text-blue-600 px-4 py-2 rounded-lg hover:bg-blue-50 transition-colors shadow-md text-sm sm:text-base font-medium">← Back to SPL-02</Link>
          </div>
        </div>
        <div className="text-center mb-4 sm:mb-6 px-4">
          <p className="text-sm sm:text-base md:text-lg text-gray-600 font-medium">👇 Click on any tournament category below to check out players 👇</p>
        </div>
          ...
          {Object.entries(groupedPlayers).map(([category, players]) => {
            ...
            return (
              <div key={category} id={`category-${category}`} className="bg-white rounded-xl sm:rounded-2xl shadow-lg border border-gray-200 overflow-hidden hover:shadow-xl transition-all duration-300">
                ...
                >
                  <div className={`absolute inset-0 bg-gradient-to-r ${categoryColor} opacity-0 group-hover:opacity-20 transition-opacity duration-300`} />
